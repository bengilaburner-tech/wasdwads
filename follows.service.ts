...
describe("AuthService", () => {
	...
	describe("loginUser", () => {
		...
		it("migrates legacy SHA-256 password hashes to modern scrypt on successful login", async () => {
			...
			const migrated = await db.select().from(users).where(eq(users.id, id)).get();
			expect(migrated).toBeDefined();
			expect(migrated?.passwordHash).toMatch(/^scrypt\$/);
			expect(migrated?.passwordHash).not.toBe(legacyHash);
		});
