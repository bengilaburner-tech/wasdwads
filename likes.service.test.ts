...
import { generateId, hashPassword, isModernPasswordHash, verifyPassword } from "./utils";
...
export async function loginUser(input: LoginInput) {
	...
	if (!isModernPasswordHash(user.passwordHash)) {
		...
		await db.update(users).set({ passwordHash: migratedHash }).where(eq(users.id, user.id));
	}
